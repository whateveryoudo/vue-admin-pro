<template>
  <!--v-if="tabs.length > 0"-->
  <div
    class="tab-menu-container"
    ref="menu"
    v-if="!hideTabs && tabs.length > 0"
  >
...
export default {
  ...
  computed: {
    ...mapState({
      tabs: (state) => state.tabMenus.tabs,
      mainNavPath: (state) => state.permission.mainNavPath,
      leftRoutes: (state) => state.permission.leftRoutes,
  ...
  methods: {
    ...
    initTabs () {
      // 获取所有设置了固定显示的tab
      const affixTabs = this.filterAffixTabs(this.leftRoutes);
      ...
      if (routes.length > 0) {
        routes.forEach((route) => {
          ...
          if (children && children.length > 0) {
            const tempTabs = this.filterAffixTabs([...children], relativePath);
